...
export const ChatPage = () => {
    ...
    // Use relative URL since Vite proxy is set for WebSocket
    const registerUser = () => {
    ...
    const onPublicMessageReceived = (payload) => {
        ...
        switch (payloadData.status) {
            case "JOIN":
                ...
                break;
            case "MESSAGE":
                setPublicChats((prevChats) => [...prevChats, payloadData]);  // Use functional form for correct state updates
                break;
            default:
                break;
        }
    ...
    const onPrivateMessageReceived = (payload) => {
        let payloadData = JSON.parse(payload.body);  // Access body to get payload
        if (privateChats.get(payloadData.senderName)) {
    ...
    const sendPrivateMessage = () => {
        if (stompClient) {
            ...
            stompClient.send('/app/private-message', {}, JSON.stringify(chatMessage));  // Use private-message endpoint
            setUserData({ ...userData, "message": "" });
        }
    };
